...
export default function App() {
  ...
  const [newProductForm, setNewProductForm] = useState({
    ...
    cost_price: 0,
    selling_price: 0,
    image_url: '/assets/products/unknown.png',
    telugu_name: '',
    hindi_name: ''
  });
  ...
  const handleCreateProduct = async (event) => {
    ...
    const payload = {
      ...
      slug: (newProductForm.slug || newProductForm.name).trim().toLowerCase().replace(/\s+/g, '-'),
      category: newProductForm.category || 'General Grocery',
      current_stock: Number(newProductForm.current_stock || 0),
      unit: newProductForm.unit || 'units',
      min_stock: Number(newProductForm.min_stock || 0),
      avg_daily_sales: Number(newProductForm.avg_daily_sales || 1),
      cost_price: Number(newProductForm.cost_price || 0),
      selling_price: Number(newProductForm.selling_price || 0),
      image_url: newProductForm.image_url || '/assets/products/unknown.png',
      telugu_name: newProductForm.telugu_name || null,
      hindi_name: newProductForm.hindi_name || null
    };

    if (!payload.name) {
      setLastActionMessage('⚠️ Please enter a product name.');
      return;
    }

    try {
      const res = await createProduct(payload);
      setLastActionMessage(res.message || 'Product added successfully.');
      ...
      setLastActionMessage('⚠️ Could not add the new product.');
    }
  };
  ...
  const handleRemoveSelectedProduct = async () => {
    if (!selectedRemoveProductId) {
      setLastActionMessage('⚠️ Please choose a product to remove.');
      return;
    }

    await handleDeleteProduct(Number(selectedRemoveProductId));
  ...
  return (
    <div className="min-h-screen flex flex-col bg-stone-50">
      <Header
        ...
        onOpenAddProduct={userRole === 'owner' ? () => setIsAddProductOpen(true) : undefined}
        onOpenRemoveProduct={userRole === 'owner' ? () => setIsRemoveProductOpen(true) : undefined}
      ...
      {isAddProductOpen && userRole === 'owner' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-stone-900/40 p-4">
          <div className="w-full max-w-2xl rounded-3xl bg-white border border-stone-200 shadow-2xl p-5">
            <div className="flex items-center justify-between mb-4">
              <div>
                <p className="text-[10px] uppercase tracking-[0.22em] text-stone-400 font-black">Owner tools</p>
                <h3 className="text-xl font-black text-stone-900">Add product</h3>
              </div>
              <button
                onClick={() => setIsAddProductOpen(false)}
                className="rounded-xl border border-stone-200 px-2.5 py-1.5 text-sm font-bold text-stone-600"
              >
                Close
              </button>
            </div>
            ...
            <form onSubmit={handleCreateProduct} className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <label className="md:col-span-2">
                <span className="block text-[11px] font-bold text-stone-500 uppercase mb-1">Product name</span>
                <input value={newProductForm.name} onChange={(e) => setNewProductForm({ ...newProductForm, name: e.target.value })} className="w-full rounded-xl border border-stone-200 bg-stone-50 px-3 py-2 text-sm" required />
              </label>

              <label>
                <span className="block text-[11px] font-bold text-stone-500 uppercase mb-1">Slug</span>
                <input value={newProductForm.slug || newProductForm.name} onChange={(e) => setNewProductForm({ ...newProductForm, slug: e.target.value })} className="w-full rounded-xl border border-stone-200 bg-stone-50 px-3 py-2 text-sm" />
              </label>

              <label>
                <span className="block text-[11px] font-bold text-stone-500 uppercase mb-1">Category</span>
                <input value={newProductForm.category} onChange={(e) => setNewProductForm({ ...newProductForm, category: e.target.value })} className="w-full rounded-xl border border-stone-200 bg-stone-50 px-3 py-2 text-sm" />
              </label>

              <label>
                <span className="block text-[11px] font-bold text-stone-500 uppercase mb-1">Current stock</span>
                <input type="number" value={newProductForm.current_stock} onChange={(e) => setNewProductForm({ ...newProductForm, current_stock: e.target.value })} className="w-full rounded-xl border border-stone-200 bg-stone-50 px-3 py-2 text-sm" />
              </label>

              <label>
                <span className="block text-[11px] font-bold text-stone-500 uppercase mb-1">Unit</span>
                <input value={newProductForm.unit} onChange={(e) => setNewProductForm({ ...newProductForm, unit: e.target.value })} className="w-full rounded-xl border border-stone-200 bg-stone-50 px-3 py-2 text-sm" />
              </label>

              <label>
                <span className="block text-[11px] font-bold text-stone-500 uppercase mb-1">Min stock</span>
                <input type="number" value={newProductForm.min_stock} onChange={(e) => setNewProductForm({ ...newProductForm, min_stock: e.target.value })} className="w-full rounded-xl border border-stone-200 bg-stone-50 px-3 py-2 text-sm" />
              </label>

              <label>
                <span className="block text-[11px] font-bold text-stone-500 uppercase mb-1">Avg daily sales</span>
                <input type="number" value={newProductForm.avg_daily_sales} onChange={(e) => setNewProductForm({ ...newProductForm, avg_daily_sales: e.target.value })} className="w-full rounded-xl border border-stone-200 bg-stone-50 px-3 py-2 text-sm" />
              </label>

              <label>
                <span className="block text-[11px] font-bold text-stone-500 uppercase mb-1">Cost price</span>
                <input type="number" value={newProductForm.cost_price} onChange={(e) => setNewProductForm({ ...newProductForm, cost_price: e.target.value })} className="w-full rounded-xl border border-stone-200 bg-stone-50 px-3 py-2 text-sm" />
              </label>

              <label>
                <span className="block text-[11px] font-bold text-stone-500 uppercase mb-1">Selling price</span>
                <input type="number" value={newProductForm.selling_price} onChange={(e) => setNewProductForm({ ...newProductForm, selling_price: e.target.value })} className="w-full rounded-xl border border-stone-200 bg-stone-50 px-3 py-2 text-sm" />
              </label>

              <label className="md:col-span-2">
                <span className="block text-[11px] font-bold text-stone-500 uppercase mb-1">Image URL</span>
                <input value={newProductForm.image_url} onChange={(e) => setNewProductForm({ ...newProductForm, image_url: e.target.value })} className="w-full rounded-xl border border-stone-200 bg-stone-50 px-3 py-2 text-sm" />
              </label>

              <div className="md:col-span-2 flex justify-end gap-2 pt-2">
                <button type="button" onClick={() => setIsAddProductOpen(false)} className="rounded-xl border border-stone-200 px-4 py-2 text-sm font-bold text-stone-600">Cancel</button>
                <button type="submit" className="rounded-xl bg-emerald-600 px-4 py-2 text-sm font-bold text-white">Save product</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {isRemoveProductOpen && userRole === 'owner' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-stone-900/40 p-4">
          <div className="w-full max-w-lg rounded-3xl bg-white border border-stone-200 shadow-2xl p-5">
            <div className="flex items-center justify-between mb-4">
              <div>
                <p className="text-[10px] uppercase tracking-[0.22em] text-stone-400 font-black">Owner tools</p>
                <h3 className="text-xl font-black text-stone-900">Remove product</h3>
              </div>
              <button
                onClick={() => setIsRemoveProductOpen(false)}
                className="rounded-xl border border-stone-200 px-2.5 py-1.5 text-sm font-bold text-stone-600"
              >
                Close
              </button>
            ...
            <div className="space-y-4">
              <label className="block">
                <span className="block text-[11px] font-bold text-stone-500 uppercase mb-1">Select product</span>
                <select
                  value={selectedRemoveProductId}
                  onChange={(e) => setSelectedRemoveProductId(e.target.value)}
                  className="w-full rounded-xl border border-stone-200 bg-stone-50 px-3 py-2 text-sm text-stone-800"
                >
              ...
              <button
                ...
                className="w-full rounded-xl bg-red-600 px-4 py-3 text-sm font-bold text-white disabled:bg-red-300 disabled:cursor-not-allowed"
              >
